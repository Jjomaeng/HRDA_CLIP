...
class EncoderDecoder(BaseSegmentor):
    ...
    def generate_pseudo_label(self, img, img_metas,return_context = False):
        return self.encode_decode(img, img_metas,return_context)

    def encode_decode(self, img, img_metas, return_context=False):
        """Encode images with backbone and decode into a semantic segmentation
        map of the same size as input."""
        x = self.extract_feat(img)

        if return_context:
            out, context = self._decode_head_forward_test(x, img_metas, return_context)
            out = resize(
                input=out,
                size=img.shape[2:],
                mode='bilinear',
        ...
        out = self._decode_head_forward_test(x, img_metas, return_context)
    ...
    def _decode_head_forward_test(self, x, img_metas, return_context=False):
        ...
        inference."""
        if return_context:
            seg_logits, context = self.decode_head.forward_test(x, img_metas, self.test_cfg, return_context)
            return seg_logits, context

        seg_logits = self.decode_head.forward_test(x, img_metas, self.test_cfg, return_context)
        return seg_logits
