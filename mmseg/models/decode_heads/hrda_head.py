...
class HRDAHead(BaseDecodeHead):
    ...
    def forward(self, inputs,return_context = False):
        ...
        if return_context:
            lr_seg,context = self.head(lr_inp,return_context)
        else :
        ...
        if return_context:
            return fused_seg, lr_seg, hr_seg,context
    ...
    def forward_test(self, inputs, img_metas, test_cfg,return_context = False):
        ...
        if return_context:
            out = self.forward(inputs, return_context)
            return out[0],out[3]
        return self.forward(inputs,return_context)[0]
